...
async function findWaterMetrics() {
  console.log('💧 Finding ALL Water-Related Metrics\n');
  ...
  // Find water-related metrics by name or category
  const waterRelated = allMetrics.filter(m => {
    ...
    return nameLC.includes('water') ||
           nameLC.includes('discharge') ||
           nameLC.includes('effluent') ||
           nameLC.includes('wastewater') ||
           categoryLC.includes('water') ||
           categoryLC.includes('discharge') ||
           categoryLC.includes('effluent');
  });

  console.log(`\n\n💧 Water-Related Metrics Found: ${waterRelated.length}\n`);

  waterRelated.forEach((m, i) => {
  ...
  if (waterRelated.length > 0) {
    ...
    const waterMetricIds = waterRelated.map(m => m.id);
    ...
    console.log(`Total water data records: ${allData.length}\n`);
    ...
    if (allData.length > 0) {
      ...
      byMetric.forEach((records, metricId) => {
        const metric = waterRelated.find(m => m.id === metricId);
...
findWaterMetrics().catch(console.error);
