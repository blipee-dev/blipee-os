/**
 * Regenerate 2025 Water & Effluents Forecasts BY SOURCE
 *
 * IMPORTANT: This forecasts actual consumption (m³, liters) for each water source,
...
async function regenerateWaterForecasts() {
  console.log('💧 Regenerating 2025 Water & Effluents Forecasts BY SOURCE\n');
  console.log('=' + '='.repeat(79) + '\n');

  // Step 1: Get all water-related metrics
  console.log('📋 Step 1: Fetching water & effluents metrics...\n');

  // Get ALL metrics first, then filter by name (since water metrics are under "Purchased Goods & Services")
  ...
  if (!waterMetrics || waterMetrics.length === 0) {
    console.error('❌ No water metrics found');
    return;
  }

  console.log(`✅ Found ${waterMetrics.length} water & effluents metrics\n`);

  waterMetrics.forEach((m, i) => {
    console.log(`  ${i + 1}. ${m.name} (${m.category}) - ${m.unit}`);
  });

  // Step 2: Delete existing 2025 water forecasts
  console.log('\n\n🗑️  Step 2: Deleting old 2025 water forecasts...\n');
  ...
  console.log(`✅ Deleted ${count || 0} old water forecast records\n`);

  // Step 3: Generate forecasts for each water metric
  console.log('🤖 Step 3: Generating water consumption forecasts...\n');
  ...
  for (const metric of waterMetrics) {
      ...
      try {
        ...
        forecast.forecasted.forEach((forecastedConsumption, i) => {
          ...
          forecastsToInsert.push({
            ...
            value: forecastedConsumption,  // ✅ CORRECT: Store actual consumption (m³, liters)
            ...
            metadata: {
              ...
              forecast_type: 'water_consumption',
    ...
    if (metricsProcessed % 5 === 0) {
      console.log(`   Processed ${metricsProcessed}/${waterMetrics.length} water metrics...`);
    }
  }

  console.log(`\n✅ Generated ${totalForecasts} water forecast records\n`);

  // Step 4: Insert forecasts in batches
  console.log('💾 Step 4: Inserting water forecasts into database...\n');
  ...
  console.log('\n🎉 Water forecast regeneration complete!\n');
  console.log('Summary:');
  ...
  console.log(`  Forecast type: Water CONSUMPTION (not emissions)`);
...
regenerateWaterForecasts().catch(console.error);
