...
async function compareMonthlyEnergy() {
  console.log('⚡ Monthly Energy Consumption Comparison: 2022-2025\n');
  console.log('=' + '='.repeat(79) + '\n');

  // First, get all energy-related metrics
  console.log('📥 Fetching energy metrics...\n');
  ...
  console.log(`Found ${metrics.length} energy-related metrics:\n`);
  metrics.forEach((m, i) => {
    console.log(`  ${i + 1}. ${m.name} (${m.category}) - ${m.unit}`);
  });

  const energyMetricIds = metrics.map(m => m.id);

  // Fetch all energy data with pagination
  console.log('\n📥 Fetching all energy records...\n');
  ...
  console.log(`Total energy records: ${allData.length}\n`);

  // Group by year-month for consumption (kWh/MWh)
  ...
  for (let month = 1; month <= 12; month++) {
    ...
    // Consumption in MWh
    consumption2022.push((consumptionByYearMonth.get(`2022-${monthStr}`) || 0) / 1000);
    consumption2023.push((consumptionByYearMonth.get(`2023-${monthStr}`) || 0) / 1000);
    consumption2024.push((consumptionByYearMonth.get(`2024-${monthStr}`) || 0) / 1000);
    consumption2025.push((consumptionByYearMonth.get(`2025-${monthStr}`) || 0) / 1000);
  ...
  for (let i = 0; i < 12; i++) {
    ...
    console.log(
      `| ${month}   | ` +
      `${val2022.toFixed(1).padStart(7)} MWh | ` +
      `${val2023.toFixed(1).padStart(7)} MWh | ` +
      `${val2024.toFixed(1).padStart(7)} MWh | ` +
      `${val2025.toFixed(1).padStart(7)} MWh | ` +
  ...
  console.log(
    `| **TOTAL** | ` +
    `**${totalConsumption2022.toFixed(1).padStart(5)} MWh** | ` +
    `**${totalConsumption2023.toFixed(1).padStart(5)} MWh** | ` +
    `**${totalConsumption2024.toFixed(1).padStart(5)} MWh** | ` +
    `**${totalConsumption2025.toFixed(1).padStart(5)} MWh** | ` +
  ...
  console.log('\n\n📊 Monthly Energy EMISSIONS Comparison (2022-2025)\n');
  console.log('| Month | 2022       | 2023       | 2024       | 2025       | Δ 22→23  | Δ 23→24  | Δ 24→25  |');
  console.log('|-------|------------|------------|------------|------------|----------|----------|----------|');
  ...
  console.log(`2022 Total: ${totalConsumption2022.toFixed(1)} MWh`);
  console.log(`2023 Total: ${totalConsumption2023.toFixed(1)} MWh (${totalChangeC2223 >= 0 ? '+' : ''}${totalChangeC2223.toFixed(1)}% vs 2022)`);
  console.log(`2024 Total: ${totalConsumption2024.toFixed(1)} MWh (${totalChangeC2324 >= 0 ? '+' : ''}${totalChangeC2324.toFixed(1)}% vs 2023)`);
  console.log(`2025 Total: ${totalConsumption2025.toFixed(1)} MWh (${totalChangeC2425 >= 0 ? '+' : ''}${totalChangeC2425.toFixed(1)}% vs 2024)`);
  ...
  // Energy intensity
  console.log('\n\n📈 Energy Intensity (tCO2e/MWh):\n');
  console.log(`2022: ${(totalEmissions2022 / totalConsumption2022).toFixed(3)} tCO2e/MWh`);
  console.log(`2023: ${(totalEmissions2023 / totalConsumption2023).toFixed(3)} tCO2e/MWh`);
  console.log(`2024: ${(totalEmissions2024 / totalConsumption2024).toFixed(3)} tCO2e/MWh`);
  console.log(`2025: ${(totalEmissions2025 / totalConsumption2025).toFixed(3)} tCO2e/MWh`);

  // Notable findings
  console.log('\n\n🔍 Notable Energy Insights:\n');

  const intensityChange2425 = ((totalEmissions2025 / totalConsumption2025) - (totalEmissions2024 / totalConsumption2024)) / (totalEmissions2024 / totalConsumption2024) * 100;

  console.log(`Grid Intensity Change (2024→2025): ${intensityChange2425 >= 0 ? '+' : ''}${intensityChange2425.toFixed(1)}%`);

  if (intensityChange2425 < 0) {
    console.log('✅ Grid is getting cleaner (lower emissions per MWh)');
  } else {
    console.log('⚠️  Grid is getting dirtier (higher emissions per MWh)');
  }
}

compareMonthlyEnergy().catch(console.error);
