...
async function main() {
  const pdfPath = '/tmp/galp-standards/report.pdf';
  const companyName = 'Galp Energia (Standards Report)';
  ...
  const filename = 'galp-energia-standards.json';
  const filepath = resolve(outputDir, filename);

  const output = {
    company_name: companyName,
    report_year: 2024,
    framework: 'GRI/SASB/WEF/TCFD',
