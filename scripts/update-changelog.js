...
function generateChangelogEntry(version, groupedCommits, releaseNotes = '') {
  ...
  // Order categories
  const categoryOrder = ['Added', 'Changed', 'Deprecated', 'Removed', 'Fixed', 'Security', 'Performance', 'Testing', 'Documentation', 'Maintenance'];
...
async function main() {
  try {
    ...
    const changelogEntry = generateChangelogEntry(currentVersion, groupedCommits, releaseNotes);
    ...
    rl.question('\n❓ Update changelog with this entry? (y/N): ', (answer) => {
      if (answer.toLowerCase() === 'y' || answer.toLowerCase() === 'yes') {
        ...
        console.log(`git commit -m "docs: update changelog for v${currentVersion}"`);
        console.log(`git tag -a v${currentVersion} -m "Release v${currentVersion}"`);
