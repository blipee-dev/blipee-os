-- Migration: GRI 301, 304, 307, 308 Support
...
CREATE INDEX IF NOT EXISTS idx_suppliers_screening ON suppliers(environmental_screening_completed);
CREATE INDEX IF NOT EXISTS idx_suppliers_assessment ON suppliers(environmental_assessment_completed);
...
CREATE TABLE IF NOT EXISTS biodiversity_sites (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  organization_id UUID NOT NULL REFERENCES organizations(id) ON DELETE CASCADE,
  site_id UUID REFERENCES sites(id) ON DELETE SET NULL,
...
CREATE INDEX IF NOT EXISTS idx_biodiversity_sites_org ON biodiversity_sites(organization_id);
CREATE INDEX IF NOT EXISTS idx_biodiversity_sites_protected ON biodiversity_sites(in_protected_area);
CREATE INDEX IF NOT EXISTS idx_biodiversity_sites_iucn ON biodiversity_sites(iucn_species_present);
CREATE INDEX IF NOT EXISTS idx_biodiversity_sites_year ON biodiversity_sites(reporting_year);

-- RLS for biodiversity_sites
ALTER TABLE biodiversity_sites ENABLE ROW LEVEL SECURITY;

DROP POLICY IF EXISTS "Users can view biodiversity sites in their organization" ON biodiversity_sites;
...
DROP POLICY IF EXISTS "Managers can manage biodiversity sites" ON biodiversity_sites;
...
DROP TRIGGER IF EXISTS update_biodiversity_sites_updated_at ON biodiversity_sites;
