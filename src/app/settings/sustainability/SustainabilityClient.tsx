...
interface SustainabilityClientProps {
  initialCatalog: any;
  initialOrganizationMetrics: any[];
  initialSites: any[];
}

export default function SustainabilityClient({
  initialCatalog,
  initialOrganizationMetrics,
  initialSites
}: SustainabilityClientProps) {
  const { user } = useAuth();
  ...
  const [organizationMetrics, setOrganizationMetrics] = useState<any[]>(initialOrganizationMetrics);
  ...
  useEffect(() => {
    // Initialize selected metrics from organization metrics
    if (organizationMetrics) {
      const selected = new Set(organizationMetrics.map((m: any) => m.metric_id));
