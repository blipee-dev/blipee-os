/**
 * Power BI API Endpoint - Business Travel Data
 *
 * Fornece dados de viagens corporativas (air, rail) formatados para Power BI
 *
 * Autenticação: API Key via header 'x-api-key'
 * Categoria incluída: Business Travel (Air, Rail)
 */
...
export async function GET(request: NextRequest) {
  try {
    ...
    let query = supabase
      ...
      .eq('metrics_catalog.category', 'Business Travel')
    ...
    if (error) {
      console.error('[PowerBI API Travel] Error:', error);
    ...
    const powerBiData = metricsData?.map(metric => ({
      ...
      travel_type: metric.metrics_catalog?.subcategory, // Air or Rail
      ...
      distance_per_employee: metric.sites?.total_employees
      ...
      emissions_per_employee: (metric.co2e_emissions && metric.sites?.total_employees)
        ...
        : null,
      emissions_per_km: metric.co2e_emissions
        ? (metric.co2e_emissions / 1000) / parseFloat(metric.value)
        : null,
    ...
    // 5. Totais
    const totals = {
      total_distance_km: powerBiData.reduce((sum, item) =>
        item.unit?.toLowerCase().includes('km') ? sum + item.value : sum, 0),
      ...
      travel_types: Array.from(new Set(powerBiData.map(item => item.travel_type))),
      // Breakdown por tipo de viagem
      by_type: Array.from(new Set(powerBiData.map(item => item.travel_type))).map(type => ({
        type,
        total_km: powerBiData
          .filter(item => item.travel_type === type && item.unit?.toLowerCase().includes('km'))
          .reduce((sum, item) => sum + item.value, 0),
        total_emissions_tco2e: powerBiData
          .filter(item => item.travel_type === type)
          .reduce((sum, item) => sum + (item.co2e_emissions || 0), 0)
      }))
    };
    ...
    console.error('[PowerBI API Travel] Unexpected error:', error);
