/**
 * Power BI API Endpoint - Energy Data
 *
 * Fornece dados de energia (eletricidade e térmica) formatados para Power BI
 *
 * Autenticação: API Key via header 'x-api-key'
 * Categorias incluídas: Electricity, Purchased Energy
 */
...
export async function GET(request: NextRequest) {
  try {
    ...
    if (error) {
      console.error('[PowerBI API Energy] Error:', error);
    ...
    const powerBiData = metricsData?.map(metric => ({
      ...
      energy_per_employee: metric.sites?.total_employees
        ? parseFloat(metric.value) / metric.sites.total_employees
        : null,
      energy_per_sqm: metric.sites?.total_area_sqm
    ...
    // 5. Totais
    const totals = {
      total_energy_consumption: powerBiData.reduce((sum, item) =>
        item.unit?.toLowerCase().includes('kwh') ? sum + item.value : sum, 0),
    ...
    console.error('[PowerBI API Energy] Unexpected error:', error);
