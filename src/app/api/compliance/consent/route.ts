...
export async function GET((_request: NextRequest) {
  try {
    ...
    return NextResponse.json(
      { _error: error instanceof Error ? error.message : 'Failed to get consents' },
      { status: 500 }
    );
  }
}
...
export async function POST((_request: NextRequest) {
  try {
    ...
    if (!type || !status) {
      return NextResponse.json(
        { _error: 'Type and status are required' },
        { status: 400 }
      );
    }
    ...
    return NextResponse.json(
      { _error: error instanceof Error ? error.message : 'Failed to record consent' },
      { status: 500 }
    );
  }
}