...
/**
 * Unified Energy Targets API
 *
 * Replaces: /api/sustainability/targets/by-category-dynamic
 * Uses: UnifiedSustainabilityCalculator for consistent calculations
 *
 * GET /api/sustainability/targets/unified-energy?organizationId=xxx&categories=Electricity,...
 *
 * Returns:
 * {
 *   success: true,
 ...
 *       targetEmissions: 151.8,
 *       currentEmissions: 180.5,
 *       projected2025FullYear: 225.0,
 *       progress: { ... }
 *     }
 *   ],
 *   overall: {
...
export async function GET(request: NextRequest) {
  try {
    ...
    const reductionPercent = targetConfig.energy_reduction_percent;
    ...
    if (!baseline || !target || !projected) {
      return NextResponse.json({
        ...
        hint: 'Ensure organization has energy data for baseline year'
      }, { status: 500 });
    }
    ...
    console.error('Error in unified-energy API:', error);
    return NextResponse.json(
      {
...
/**
 * Get category-level breakdown for energy metrics
 */
  ...
  const results = await Promise.all(
    categories.map(async (category) => {
      ...
      const currentEmissions = currentYearData
        .reduce((sum, d) => sum + parseFloat(d.co2e_emissions || '0'), 0) / 1000;
