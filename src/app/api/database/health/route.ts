...
// Prevent this route from being statically rendered
...
export async function GET(request: NextRequest) {
  try {
    ...
    switch (action) {
      ...
      case 'repair':
        ...
        if (member.role !== 'platform_admin') {
          return NextResponse.json({ 
            error: 'Platform admin privileges required for auto-repair' 
          }, { status: 403 });
        }
      ...
      default:
        return NextResponse.json({ 
          error: 'Invalid action. Valid actions: status, validate, report, repair' 
        }, { status: 400 });
    }
