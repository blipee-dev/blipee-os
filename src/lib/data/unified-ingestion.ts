...
import { weatherAPI, carbonMarketAPI, regulatoryAPI, gridAPI } from './external-apis'
...
export class UnifiedDataIngestion {
  ...
  async ingestAllData(organizationId: string): Promise<{
    ...
    console.log('🌍 Starting unified data ingestion for organization:', organizationId)
    ...
    const unifiedData: UnifiedData = {
      ...
      emissions: this.mergeEmissionsData(documentData.emissions, internalData.emissions),
      external: {
        weather: weatherData,
    ...
    const recommendations = await this.generateRecommendations(unifiedData, insights)
  ...
  private async ingestWeatherData(organizationId: string): Promise<any> {
    try {
      ...
      const locations = (!dbLocations || dbLocations.length === 0) 
        ? [{ latitude: 40.7128, longitude: -74.0060, name: 'Default' }]
        : dbLocations
      ...
      return {
        ...
        aggregated: {
          avgTemperature: this.average(weatherResults.map(w => w.current.temperature)),
          totalHeatingDemand: this.sum(weatherResults.map(w => w.current.heatingDemand)),
          totalCoolingDemand: this.sum(weatherResults.map(w => w.current.coolingDemand)),
          solarPotential: this.average(weatherResults.map(w => w.current.solarGeneration))
        },
        energyImpact,
  ...
  private async ingestRegulatoryData(organizationId: string): Promise<any> {
    try {
      ...
      return {
        ...
        overallCompliance: this.average(complianceChecks.map(c => c.compliance.overallCompliance))
      }
    } catch (error) {
  ...
  private async ingestGridData(organizationId: string): Promise<any> {
    try {
      ...
      const currentEmissions = currentUsage * gridData.current.carbonIntensity / 1000
      const optimalEmissions = currentUsage * gridData.optimal.lowestIntensity.intensity / 1000
  ...
  private async ingestDocumentData(organizationId: string): Promise<any> {
    try {
      ...
      if (parseResults.emissions.total > 0) {
        await this.storeDocumentEmissions(organizationId, parseResults.emissions)
      }
   ...
   * Generate insights from unified data
   */
  private async generateDataInsights(data: UnifiedData): Promise<DataInsight[]> {
    ...
    if (data.external.grid?.savings?.percentage > 10) {
      insights.push({
        ...
        impact: 'medium',
        dataPoints: ['carbon intensity', 'optimal times', 'renewable percentage'],
        actionable: true
      })
    }
  ...
  private async generateRecommendations(
    ...
    if (data.external.weather?.aggregated?.totalCoolingDemand > 20) {
      recommendations.push('Pre-cool buildings during off-peak hours to reduce peak demand')
    }
    
    // Market-based recommendations
    if (data.external.carbonMarket?.trends?.voluntary?.direction === 'up') {
      recommendations.push('Lock in carbon offset prices now before further increases')
    }
    ...
    if (criticalDeadlines?.length > 0) {
      recommendations.push(`Complete ${criticalDeadlines[0].regulation} requirements - ${criticalDeadlines[0].daysRemaining} days remaining`)
    }
    
    // Grid recommendations
    if (data.external.grid?.optimal?.recommendations) {
      recommendations.push(...data.external.grid.optimal.recommendations.slice(0, 2))
    }
    
    // Emission reduction recommendations
    if (data.emissions.trend === 'increasing') {
      recommendations.push('Emissions trending upward - implement immediate reduction measures')
    }
  ...
  private calculateWeatherEnergyImpact(weatherData: any[]): any {
    const totalHeating = this.sum(weatherData.map(w => w.current.heatingDemand))
    const totalCooling = this.sum(weatherData.map(w => w.current.coolingDemand))
  ...
  private mergeEmissionsData(documentEmissions: any, internalEmissions: any): any {
    return {
      total: (documentEmissions?.total || 0) + (internalEmissions?.total || 0),
      byScope: {
        scope1: (documentEmissions?.byScope?.scope1 || 0) + (internalEmissions?.scope1 || 0),
        scope2: (documentEmissions?.byScope?.scope2 || 0) + (internalEmissions?.scope2 || 0),
        scope3: (documentEmissions?.byScope?.scope3 || 0) + (internalEmissions?.scope3 || 0)
      },
  ...
  private async storeUnifiedData(organizationId: string, data: UnifiedData): Promise<void> {
    await this.supabase
  ...
  private async storeDocumentEmissions(organizationId: string, emissions: any): Promise<void> {
    const emissionRecords = Object.entries(emissions.byDocument).map(([fileName, amount]) => ({
      organization_id: organizationId,
  ...
  private async getEmissionsData(organizationId: string): Promise<any> {
    const { data } = await this.supabase
      ...
      .select('*')
      .eq('organization_id', organizationId)
      .gte('emission_date', new Date(Date.now() - 30 * 24 * 60 * 60 * 1000).toISOString())
    
    if (!data) return {}
    
    const total = data.reduce((sum: number, e: any) => sum + e.emissions_amount, 0)
...
interface RealTimeUpdate {
  type: 'grid_update' | 'weather_update' | 'document_processed' | 'market_update'
  data: any
  timestamp: string
}
