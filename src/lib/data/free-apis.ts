  ...
  async searchFactors(query: string): Promise<any[]> {
    try {
      const response = await fetch(
        `${this.baseUrl}/emission-factors/search?query=${query}`,
        {
          headers: {
            Authorization: `Bearer ${this.apiKey}`,
          },
...
export class ClimatePolicyRadarClient {
  ...
  async getCountryRegulations(country: string): Promise<any> {
    try {
      const response = await fetch(
        `${this.baseUrl}/countries/${country}/policies`,
      );
      return await response.json();
    } catch (error) {
...
export class ClimateWatchAPIClient {
  ...
  async getCountryEmissions(country: string): Promise<any> {
    try {
      const response = await fetch(
        `${this.baseUrl}/emissions?country=${country}`,
      );
      return await response.json();
    } catch (error) {
...
export class RegionalGridAPIs {
  ...
  async getCAISOData(): Promise<any> {
    try {
      ...
      const response = await fetch(
        "http://www.caiso.com/outlook/SP/current_renewables.json",
      );
      return await response.json();
    } catch (error) {
      return null;
...
export class FreeAPIOrchestrator {
  ...
  constructor(apiKeys?: { climatiq?: string; carbonInterface?: string }) {
    if (apiKeys?.climatiq) {
