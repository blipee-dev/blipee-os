...
export class EmissionFactorsDatabase {
  ...
  private static readonly ENERGY_FACTORS = {
    ...
    renewable: {
      ...
      hydro: 0.024,
      nuclear: 0.012,
      biomass: 0.230, // Depends on source
      geothermal: 0.038
    }
  }
    ...
    switch (activity.type) {
      case 'energy':
        if (activity.category === 'electricity') {
          ...
          factor = this.ENERGY_FACTORS.fuel[activity.subcategory as keyof typeof this.ENERGY_FACTORS.fuel] || 0
          scope = 1
        }
        break
        
      case 'transport':
        factor = this.getTransportFactor(activity.category, activity.subcategory)
        scope = activity.category === 'company_fleet' ? 1 : 3
        break
        
      case 'waste':
        const wasteCategory = this.WASTE_FACTORS[activity.category as keyof typeof this.WASTE_FACTORS]
        factor = activity.subcategory && wasteCategory ? (wasteCategory as any)[activity.subcategory] || 0 : 0
        break
        
      case 'water':
        const waterCategory = this.WATER_FACTORS[activity.category as keyof typeof this.WATER_FACTORS]
        factor = activity.subcategory && waterCategory ? (waterCategory as any)[activity.subcategory] || 0.344 : 0.344
        break
        
      case 'materials':
        factor = this.getMaterialFactor(activity.category, activity.subcategory)
        break
    }
  ...
  static getElectricityFactor(location?: string): number {
    ...
    return countryFactors[location] || this.ENERGY_FACTORS.electricity.global.default
  }
  ...
  static getTransportFactor(mode: string, type?: string): number {
    const modeData = this.TRANSPORT_FACTORS[mode as keyof typeof this.TRANSPORT_FACTORS]
    ...
    if (type && typeof modeData === 'object' && type in modeData) {
      ...
      if (typeof typeData === 'object') {
        return (typeData as any).economy || Object.values(typeData)[0] as number
      }
    }
  ...
  static getMaterialFactor(category: string, type?: string): number {
    const categoryData = this.MATERIAL_FACTORS[category as keyof typeof this.MATERIAL_FACTORS]
    if (!categoryData) return 0
  ...
  private static getUnit(category: string, subcategory: string): string {
    ...
    return units[category]?.[subcategory] || units[category]?.default || 'kgCO2e/unit'
  }
}
