...
export const ONBOARDING_STEPS: OnboardingStep[] = [
  // Step 1: Organization Type (30 seconds)
  {
    ...
    type: 'select',
    options: [
      { value: 'corporate', label: 'Corporate/Enterprise', description: 'Multi-location businesses' },
      { value: 'manufacturing', label: 'Manufacturing', description: 'Production facilities' },
      { value: 'real_estate', label: 'Real Estate', description: 'Property management' },
      { value: 'hospitality', label: 'Hospitality', description: 'Hotels, restaurants' },
      { value: 'healthcare', label: 'Healthcare', description: 'Hospitals, clinics' },
      ...
      { value: 'nonprofit', label: 'Non-Profit', description: 'NGOs, foundations' }
    ],
  ...
  // Step 6: Emission Scopes (30 seconds)
  {
    ...
    type: 'multiselect',
    options: [
      { value: 'scope1', label: 'Scope 1', description: 'Direct emissions (vehicles, facilities)' },
      { value: 'scope2', label: 'Scope 2', description: 'Electricity and energy' },
      { value: 'scope3', label: 'Scope 3', description: 'Supply chain and indirect' }
    ],
    validation: { required: true },
    estimatedTime: 30
  },
...
export class OnboardingOrchestrator {
  ...
  getProgress(): { percentage: number; timeElapsed: number; estimatedRemaining: number } {
    const completedSteps = this.responses.size
    const totalSteps = ONBOARDING_STEPS.length
    const percentage = Math.round((completedSteps / totalSteps) * 100)
    
    const timeElapsed = Math.round((Date.now() - this.startTime) / 1000)
    const avgTimePerStep = completedSteps > 0 ? timeElapsed / completedSteps : 30
