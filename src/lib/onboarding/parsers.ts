...
export function csvBuildingParser(input: string): BuildingInput[] {
  ...
  return lines.map(line => {
    ...
    if (parts.length < 2) {
      throw new Error(`Invalid format: "${line}". Expected "Building Name, City"`)
    }
 ...
 */
export function areaAssignmentParser(input: string): Array<{ area: string; users: string[] }> {
  const lines = input
    .split('\n')
    .map(line => line.trim())
    .filter(line => line.length > 0)
 ...
 */
export function validateParsedData(type: string, data: any[]): {
  valid: boolean
  errors: string[]
} {
  const errors: string[] = []
