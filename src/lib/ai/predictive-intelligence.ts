...
export class PredictiveIntelligence {
  ...
  async predictEmissions(
    ...
    const timeSeriesPrediction = await this.runTimeSeriesModel(
      organizationId,
      horizon,
    );

    // 2. Pattern-based prediction
    const patternPrediction = await this.runPatternModel(
      organizationId,
      horizon,
    );
  ...
  private async identifyRisks(
    ...
    if (this.detectEmissionSpike(ensemble)) {
      risks.push({
        ...
        impact: "high",
        description:
          "Potential 25% emission spike in week 3 due to extreme weather",
    ...
    return risks.sort(
      (a, b) =>
        b.probability * this.impactScore(b.impact) -
        a.probability * this.impactScore(a.impact),
    );
  }
  ...
  private async identifyOpportunities(
    ...
    opportunities.push({
      ...
      description:
        "Off-peak energy shifting could reduce emissions by 150 tonnes/year",
      confidence: 0.9,
    });
    ...
    return opportunities.sort(
      (a, b) => b.potential_reduction - a.potential_reduction,
    );
  }
  ...
  private generateRecommendations(
    ...
    const highRisks = risks.filter(
      (r) => r.impact === "high" && r.probability > 0.5,
    );
    highRisks.forEach((risk) => {
      recommendations.push(
        `⚠️ ${risk.mitigation} to prevent ${risk.description}`,
      );
    });

    // Quick win opportunities
    const quickWins = opportunities.filter(
      (o) => o.implementation === "immediate" && o.confidence > 0.8,
    );
    quickWins.forEach((opp) => {
      recommendations.push(
        `💡 ${opp.description} - start immediately for ${opp.potential_reduction} tonnes reduction`,
      );
    });
    ...
    if (ensemble.trend === "increasing") {
      recommendations.push(
        "📈 Consider accelerating decarbonization initiatives - current trend is upward",
      );
    }
  ...
  async modelScenario(
    ...
    const scenarioForecast = this.applyScenarioChanges(
      baseline.forecast,
      scenario.changes,
    );
   ...
   */
  async predictInitiativeImpact(initiative: {
    type: string;
    description: string;
    timeline: string;
    investment: number;
