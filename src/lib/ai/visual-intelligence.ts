...
export class VisualIntelligence {
  ...
  async generateVisualization(
    ...
    const insights = this.generateInsights((component as any).data, intent.type)
  ...
  private async understandVisualizationIntent(request: string): Promise<{
    ...
    if (lower.includes('breakdown') || lower.includes('composition')) type = 'pie'
    ...
    if (lower.includes('heatmap') || lower.includes('intensity')) type = 'heatmap'
    if (lower.includes('target') || lower.includes('progress')) type = 'gauge'
    if (lower.includes('correlation')) type = 'scatter'
    if (lower.includes('forecast') || lower.includes('prediction')) type = 'forecast'
   ...
   */
  private async generateWidgets(focus: string, context: any): Promise<Widget[]> {
    ...
    widgets.push({
      ...
      config: {
        title: 'AI Insights',
        insights: [
          { text: 'Unusual energy spike detected in Building A', priority: 'high' },
          { text: 'Opportunity: Off-peak rates available tonight', priority: 'medium' }
        ],
  ...
  private generateInsights(data: any, type: string): string[] {
    ...
    if (type === 'pie' && data) {
      const largest = data.reduce((max: any, item: any) => item.value > max.value ? item : max)
      insights.push(`${largest.name} represents your biggest opportunity for reduction at ${largest.value}%`)
    }
    
    if (type === 'line') {
      insights.push('Emissions show a consistent downward trend of 2% per month')
      insights.push('Current trajectory will achieve 35% reduction by year-end')
    }
  ...
  private selectOptimalFormat(requirements: any): 'interactive' | 'pdf' | 'presentation' {
    if (requirements.includes('board')) return 'presentation'
    if (requirements.includes('compliance') || requirements.includes('audit')) return 'pdf'
    return 'interactive'
  }

  private async createShareableReport(sections: any[], format: string): Promise<string> {
  ...
  private createSmartVisualization(intent: any, context: any): DynamicComponent {
...
type VisualizationType = 'pie' | 'line' | 'bar' | 'sankey' | 'heatmap' | 'gauge' | 'scatter' | 'forecast'
