...
export function calculateBuildingEmissions(context: BuildingContext): EnhancedBuildingContext['sustainability'] {
  // Calculate Scope 2 emissions from electricity
  const electricityEmissions = (context.currentState.energyUsage / 1000) * EMISSION_FACTORS.electricity.grid
  ...
  const gasEmissions = hvacEnergy * gasHeatingRatio * EMISSION_FACTORS.naturalGas
  ...
  const wasteEmissions = context.currentState.occupancy * wastePerPerson * EMISSION_FACTORS.waste / 24 // hourly
  const waterEmissions = context.currentState.occupancy * waterPerPerson * EMISSION_FACTORS.water / 1000 / 24 // hourly
  
  const totalEmissions = electricityEmissions + gasEmissions + wasteEmissions + waterEmissions
  
  // Calculate breakdown
  const breakdown = [
    { source: 'Electricity', emissions: electricityEmissions, percentage: 0, scope: 2 },
  ...
  return {
    ...
    targets: {
      ...
      currentProgress: Math.min(100, (2400 - (totalEmissions * 24)) / 2400 * 100)
    }
  }
}

export function buildEnhancedContext(building: BuildingContext): EnhancedBuildingContext {
  return {
    ...building,
