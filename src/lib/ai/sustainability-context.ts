...
export async function buildSustainabilityContext(
  ...
  const emissionsByScope = emissions?.reduce((acc, e) => {
    const scope = `scope${e.scope}` as keyof typeof acc
    acc[scope] += e.emissions_amount
    return acc
  }, { scope1: 0, scope2: 0, scope3: 0 }) || { scope1: 0, scope2: 0, scope3: 0 }

  const totalEmissions = emissionsByScope.scope1 + emissionsByScope.scope2 + emissionsByScope.scope3

  // Calculate emissions by source
  const emissionsBySource = emissions?.reduce((acc, e) => {
    const existing = acc.find(s => s.source === e.source_type)
    if (existing) {
      existing.amount += e.emissions_amount
    } else {
      acc.push({ source: e.source_type, amount: e.emissions_amount, percentage: 0 })
    }
    return acc
  ...
  return {
    ...
    targets: targets?.map(t => ({
      ...
      currentProgress: calculateProgress(t.baseline_value, t.current_value || t.baseline_value, t.target_value),
      status: determineTargetStatus(t)
    })) || [],
    compliance: {
      frameworks: Array.from(new Set(compliance?.map((c: any) => c.framework) || [])),
    ...
    opportunities: generateOpportunities(emissionsBySource, modules?.map(m => m.module_id) || []),
    esgMetrics: {
      environmental: 72,
      social: 68,
      governance: 85
...
function determineTargetStatus(target: any): 'on-track' | 'at-risk' | 'off-track' {
  const progress = calculateProgress(target.baseline_value, target.current_value || target.baseline_value, target.target_value)
...
function generateOpportunities(
  emissionsBySource: Array<{ source: string; amount: number; percentage: number }>,
  enabledModules: string[]
): Array<any> {
  const opportunities = []
  
  // Generate opportunities based on emission sources
  if (emissionsBySource.find(s => s.source === 'building' && s.percentage > 30)) {
    opportunities.push({
  ...
  if (emissionsBySource.find(s => s.source === 'vehicle' && s.percentage > 20)) {
    opportunities.push({
...
- Top Emission Sources: ${context.emissions.bySource.slice(0, 3).map(s => `${s.source} (${s.percentage.toFixed(1)}%)`).join(', ')}
