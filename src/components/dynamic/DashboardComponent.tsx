...
export function DashboardComponent({ 
  ...
  const MetricCard = ({ icon: Icon, title, value, subtitle, trend, color = 'blue' }: any) => (
    <motion.div
  ...
  const AlertCard = ({ alert }: { alert: any }) => (
    ...
    >
      ...
      <div className="flex-1">
        ...
        <div className="text-xs text-white/50 capitalize">{alert.priority} priority</div>
      </div>
    </motion.div>
  )
          ...
          <>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
              <MetricCard
                ...
                trend={metrics.energy.trend === 'increasing' ? 'up' : metrics.energy.trend === 'decreasing' ? 'down' : 'stable'}
                color="blue"
              />
              <MetricCard
                icon={Thermometer}
            ...
            <div className="backdrop-blur-xl bg-white/[0.03] border border-white/[0.05] rounded-2xl p-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div className="text-center">
                  <div className="text-2xl font-bold text-blue-400">{defaultPredictions.nextHour.toLocaleString()}W</div>
                ...
                <div className="text-center">
                  ...
                  <div className="text-sm text-white/60">Potential Monthly Savings</div>
                </div>
              </div>
            </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            <div className="backdrop-blur-xl bg-white/[0.03] border border-white/[0.05] rounded-2xl p-6">
              ...
              <div className="space-y-4">
                {[
                  { name: 'HVAC', usage: Math.round(metrics.energy.currentUsage * 0.47), color: 'blue' },
                  { name: 'Lighting', usage: Math.round(metrics.energy.currentUsage * 0.28), color: 'purple' },
                  { name: 'Equipment', usage: Math.round(metrics.energy.currentUsage * 0.25), color: 'green' }
                ].map((item) => (
                  <div key={item.name} className="flex items-center justify-between">
                    ...
                    <span className="text-white/70 font-mono">{item.usage.toLocaleString()}W</span>
                  </div>
                ))}
              </div>
            </div>
              ...
              <div className="space-y-4">
                ...
                <div className="border-t border-white/10 pt-2">
                  <div className="flex justify-between font-semibold">
                    ...
                    <span className="text-green-400">${metrics.energy.cost}</span>
                  </div>
                </div>
              </div>
            </div>
          ...
          <div className="backdrop-blur-xl bg-white/[0.03] border border-white/[0.05] rounded-2xl p-6">
            ...
            <div className="grid grid-cols-2 gap-4">
              {metrics.occupancy.zones.map((zone) => (
                <div key={zone.name} className="flex items-center justify-between p-3 rounded-xl bg-white/[0.02] border border-white/[0.05]">
                  <span className="text-white">{zone.name}</span>
                  <span className="text-white/70 font-mono">{zone.occupancy} people</span>
                </div>
              ))}
            </div>
          </div>
