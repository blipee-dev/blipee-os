...
import { OnboardingOrchestrator, ONBOARDING_STEPS, OnboardingStep } from '@/lib/onboarding/quick-setup'
...
export function ConversationalOnboarding({ onComplete, userId }: ConversationalOnboardingProps) {
  ...
  const canProceed = selectedValue !== null || currentStep?.validation?.required === false
        ...
        <AnimatePresence mode="wait">
          ...
          >
            ...
            <div className="space-y-3">
              {currentStep.type === 'select' && currentStep.options?.map(option => (
                <motion.button
                  key={option.value}
              ...
              {currentStep.type === 'multiselect' && currentStep.options?.map(option => {
                ...
                return (
                  <motion.button
                    ...
                    onClick={() => {
                      ...
                      if (isSelected) {
                        setSelectedValue(current.filter((v: string) => v !== option.value))
                      } else {
